...
# =============================main loop======================================================================
    def main_loop(self):
        if self.phase == 'START':
            ...
            self.phase = 'LIFT_SHOLDER_AND_ELBOW_SAFETY'
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------------
# ======================= whipe fruits tray approach he chnace to collied with rack so lift shoulder_lift 2 and    elbow to lift up the alinf the base is better =====================================================================
# ========================================================================================================================================================================================================================= 

        elif self.phase == 'LIFT_SHOLDER_AND_ELBOW_SAFETY':
            if not self.phase_initialized:
                ...
                self.targets = {
                    ...
                    'shoulder_lift_joint': self.temp['shoulder_lift_joint'] - 0.20,
                    'elbow_joint': self.temp['elbow_joint']- 0.25,
                }
            ...
            if self.move_joint_group(self.targets, self.speed_scale):
                self.phase_initialized = False
                self.get_logger().info(f"    {self.current_tcp_orient} , pose current {self.current_tcp_pos} ,")
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if self.wait_for_timer(1.0):
                self.phase = 'FRUITS_TRAY_HOVER'   

# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

        elif self.phase  == 'FRUITS_TRAY_HOVER':
...
# ----------------------------------------------------------------------------------------------------------------------------
            ...
            if reached:
                ...
                self.get_logger().info("Lift Complete.")
                self.phase_initialized = False
                self.phase = 'BASE_ALING_TO_DUSTBIN'    #     <===  CHNAGE HERE IF NEED   DUSTBIN ALING 

# -------------------------------------------------------------------------------------------------------------------------------------------------------------

        elif self.phase == 'BASE_ALING_TO_DUSTBIN':
            # 1. Calculate the target: Initial position + 50 degrees (np.radians(50))
            if not self.phase_initialized :
            ...
            if self.move_joint_to_angle(self.target_angle, self.joint_names_list[0], 0):
                self.get_logger().info(f"Base rotated +45° to {np.degrees(self.target_angle):.2f}°. Moving to dustbin.")
                self.phase_initialized = False
...
# ----------------------------------------------------------------------------------------------------------
            ...
            if not self.phase_initialized :
                ...
                self.get_logger().info(
                    f"Distance to Dustbin -> X:   target by use {self.target_dustbin}",
                    throttle_duration_sec=0.5  
                )
