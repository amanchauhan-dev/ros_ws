#!/usr/bin/env python3
# """
# Deterministic line-by-line coverage with obstacle avoidance

# States:
# - MOVE_TO_START : go to closest endpoint of active line
# - ROTATE_TO_LINE: rotate in place to align with line
# - FOLLOW_LINE   : follow line with cross-track control
# - DONE          : stop after all lines are covered

# Key properties:
# - Each line is covered exactly once
# - Direction of each line is chosen dynamically
# - Robot never jumps far between lines
# """
...
# # ---------- CLOSED SHAPE ----------
# LINES = [
#     ((-1.5, -5.6), (0.32, -5.6)),    # index 0 (start) up
#     ((0.32, -5.6), (0.32,  1.38)), # left
#     ((0.32,  1.38), (-1.5, 1.38)), # down
#     ((-1.5, 1.38), (-1.5, -5.6)), # right
#     ((-1.5, -5.6), (-3.459, -5.6)), # down
#     ((-3.459, -5.6), (-3.459, 1.38)), # left
#     ((-3.459, 1.38), (-1.5, 1.38)), # up
#     ((-1.5,  1.38), (-1.5, -5.6)), # right
# ]
...
#         self.current_line_idx = START_LINE_INDEX
...
#         # )
#         start, end = self.lines[self.current_line_idx]
...
#             f"[INIT] Starting on line {self.current_line_idx}, "
...
#                 self.get_logger().info(
#                     f"[FOLLOW] Line {self.current_line_idx} completed "
#                     f"({self.completed_lines}/{self.total_lines})"
#                 )

#                 if self.completed_lines >= self.total_lines:
#                     self.get_logger().info("[DONE] All lines covered. Stopping.")
#                     self.cmd_pub.publish(Twist())
...
#             # Correct signed cross-track error
...
#             twist.linear.x = SPEED_FOLLOW if abs(ct_err) < MAX_CROSSTRACK else SPEED_APPROACH
