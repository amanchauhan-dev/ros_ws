...
class Ransac(Node):
    def __init__(self):
        ...
        self.global_buffer_distance_difference = 0.15
    ...
    def control_loop(self):
        ...
        if shape:
            ...
            is_valid = False
            self.get_logger().info(f"Right status: {self.right_side_next_shape} {self.right_side_next_plant_index}")
        ...
        if shape:
            ...
            cx, cy, dist = self.compute_shape_position(group)
            is_valid = False
            self.get_logger().info(f"Left status: {self.left_side_next_shape} {self.left_side_next_plant_index}")
            for idx, (px, py) in enumerate(self.plants_tracks):
