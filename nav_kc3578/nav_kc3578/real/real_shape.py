...
SECTOR_WIDTH = math.radians(15)   # ±15°
...
class Ransac(Node):
    def __init__(self):
        ...
        self.create_subscription(
            Bool,
            "/fertilizer_placement_status",
            self.fertilizer_placement_status_callback,
            10,
        )
    ...
    def lidar_callback(self, msg: LaserScan):
        ...
        for r, ang, gx, gy in zip(ranges, angles, xg, yg):
            if not (msg.range_min < r < msg.range_max):
                continue

            rel_ang = math.atan2(math.sin(ang), math.cos(ang))

            if abs(rel_ang - RIGHT_ANGLE) <= SECTOR_WIDTH:
                right_pts.append([gx, gy])
            elif abs(rel_ang - LEFT_ANGLE) <= SECTOR_WIDTH:
                left_pts.append([gx, gy])

        if self.last_point_buffered is None:
            self.right_buffer.append(self.get_side(np.array(right_pts)))
            self.left_buffer.append(self.get_side(np.array(left_pts)))
    ...
    def get_side(self, pts):
        ...
        stable = np.abs(dists - med) < 2.5 * mad
        pts = pts[stable]
