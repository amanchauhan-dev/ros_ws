...
def classify_fruit(img_wb, candidate):
    """
    [6] Classify a fruit candidate as good or bad using two weighted criteria.
    """
    H, W   = img_wb.shape[:2]
...
def compute_surface_depth(raw_depth, box_width_px, focal_x):
    """
    [8-helper] Subtract estimated fruit radius from centre depth (for X/Y only).
    """
...
class FruitsAndArucoTF(Node):
    ...
    def process_image(self):
        ...
        Only bad fruits are annotated and published as TFs.
        Good fruits are silently ignored after classification.
        """
        ...
        for cand in candidates:
            try:
                ...
                # ── CHANGED: good fruits are silently skipped, no annotation ──
                if not is_bad:
                    continue
