...
class SideWallPreview(Node):
    ...
    def visualize(self):
        ...
        if self.cur_left_buffer:
            xs, ys = zip(*self.cur_left_buffer)
            self.ax.scatter(xs, ys, c="red", s=1, label="Current Left")

        if self.cur_right_buffer:
            xs, ys = zip(*self.cur_right_buffer)
            self.ax.scatter(xs, ys, c="blue", s=1, label="Current Right")

        # accumulated points (lighter / smaller)
        if self.left_points_buffer:
            xs, ys = zip(*self.left_points_buffer)
            self.ax.scatter(xs, ys, c="orange", s=1, alpha=0.4, label="Left History")
