...
LINES = [
    ((-1.5, -5.6), (0.32, -5.6)),      # 0
    ((0.32, -5.6), (0.32,  1.38)),     # 1
    ((0.32,  1.38), (0.32, -5.6)),     # 1
    # ((0.32,  1.38), (-1.5, 1.38)),     # 2
    # ((-1.5, 1.38), (-1.5, -5.6)),      # 3
    # ((-1.5, -5.6), (-3.459, -5.6)),    # 4
    # ((-3.459, -5.6), (-3.459, 1.38)),  # 5
    # ((-3.459, 1.38), (-1.5, 1.38)),    # 6
    # ((-1.5, 1.38), (-1.5, -5.6))       # 7
]
...
class LineFollower(Node):
    ...
    def __init__(self):
        ...
        self.create_subscription(Bool, TO_STOP, self.to_stop_cb, 10)
        self.to_stop = False
        self.x = -1.5
        self.y = -5.6
        self.yaw = 0.0
    ...
    try:
        while rclpy.ok():
            ...
            for idx, ((x1,y1),(x2,y2)) in enumerate(LINES):
                ax.plot([x1,x2],[y1,y2],'gray',lw=2, alpha=0.3)
                # Label lines
                mx, my = (x1+x2)/2, (y1+y2)/2
                ax.text(mx, my, str(idx), fontsize=8, ha='center')

            # Draw completed lines in green
            for idx in range(node.line_idx):
                (x1,y1),(x2,y2) = LINES[idx]
                ax.plot([x1,x2],[y1,y2],'g-',lw=3, alpha=0.7)

            # Draw current line in blue
            if node.state != node.DONE and node.line_idx < len(LINES):
                (x1,y1),(x2,y2) = LINES[node.line_idx]
                ax.plot([x1,x2],[y1,y2],'b-',lw=4)
                ax.scatter([x1], [y1], c='blue', s=100, marker='o', label='Line Start')
                ax.scatter([x2], [y2], c='cyan', s=100, marker='s', label='Line End')
            ...
            # Draw robot
            ax.scatter(node.x, node.y, c='red', s=100, zorder=5)
            ax.arrow(
                node.x, node.y,
                0.3*math.cos(node.yaw),
                0.3*math.sin(node.yaw),
                head_width=0.15,
                color='red',
                zorder=5
            )
            ...
            # Draw lidar points
            if node.lidar_pts:
                lx, ly = zip(*node.lidar_pts)
                ax.scatter(lx, ly, c='orange', s=2, alpha=0.3)

            # Status text
            state_names = {
                node.MOVE_TO_START: "MOVE_TO_START",
                node.ROTATE_TO_LINE: "ROTATE_TO_LINE",
                node.FOLLOW_LINE: "FOLLOW_LINE",
                node.TRANSITION_STOP: "TRANSITION_STOP",
                node.DONE: "DONE"
            }
            status = (
                f"State: {state_names.get(node.state, 'UNKNOWN')}\n"
                f"Line: {node.line_idx}/{node.total_lines-1}\n"
                f"Pos: ({node.x:.2f}, {node.y:.2f})\n"
                f"Yaw: {math.degrees(node.yaw):.1f}°"
            )
            ...
            ax.set_title("Line Following Robot with Sequential Coverage")
            plt.pause(0.05)
