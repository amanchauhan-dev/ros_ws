...
# ------------------------------------------------------------------------------------------------  
            ...
            # Check if we reached the orientation quaternion defined in __init__
            if self.orient_to_target(self.fruitHomeOrientation, tol=0.03):
                self.get_logger().info("✓ Orientation Correct. Moving to Target.")
                # Fixed typo: Ensure this matches your next 'elif' block
...
# -----------------------------------------------------------------------------------------------------------------------
            ...
            self.current_fruits_pose = fruit_record['pos']
            # 2. Create a target to HOVER ABOVE the fruit   yes i 
            hover_target = self.current_fruits_pose.copy()
            hover_target[2] += 0.10
       
            if self.move_to_tcp_target(hover_target, self.approach_tol,slow=True):
                self.publish_twist([0.0, 0.0, 0.0], 0.0) # Stop briefly
                self.get_logger().info("Hover on the current index fruits . Descending now...")
                self.phase = 'CHECK_ORIENATION_FRUIT'
  
    # LIKE WE are here try to go hover on   the fruits 
    # THEN => we again check the  orientation that is maintain or not becaue the orenation of tool1 (magnet)
    # moved slowly down to target 
# -----------------------------------------------------------------------------------------------------------------------
# =====================THIS IS ORTEtION check ===============================================

        elif self.phase == 'CHECK_ORIENATION_FRUIT':
            self.get_logger().info("Checking pickup orientation...", throttle_duration_sec=2.0)
...
# --------------------------------------------------------------------------------------------
# 2 sceond approach is that in the future like we fail then  first go to 5cm then adjuct the orienation 
...
# ----------------------------------------------------------------------------------------------------------------
            ...
            if self.current_tcp_pos is None:
                self.get_logger().info("lift ke time per tcp pose nahi mil raha hai bro , return kar de raha hu ")
                return 
            target = self.current_tcp_pos.copy()
            target[2] += 10
            if self.move_to_tcp_target(target,0.01,slow=True):
                self.get_logger().info("lift hogaya matlab next phase Aling me jaunga ")
                # yaha per next phase again koi node collied ho raha to reverse kar sakte hai y-Axis ko current se negative kar de bas 
                self.phase = 'ALING_PHASE_TO_BIN'
# --------------------------------------------------------------------------------------------------------------------------------------
        elif self.phase == 'ALING_PHASE_TO_BIN':
            target = self.dustbinPosition.copy()
            target[1] += 0.1  
                                                # past experience is that the get collied with rack also they moved to fast 
            if self.align_joint_to_pose(target, "dustbin", self.joint1, 0): # Fixed variable target_pose -> target
                self.get_logger().info("dustbin ke upar hai hm log bhai ")
                self.phase = 'MOVED_DUSTBIN_POSE'

        elif self.phase == 'MOVED_DUSTBIN_POSE':
            target = self.dustbinPosition.copy()

            if self.move_to_tcp_target(target, 0.05 , slow=True):
                self.get_logger().info("time to deattach phase ")
                self.phase = 'DETACH_FRUITS' # Fixed typo in variable name match
# -------------------------------------------------------------------------------------------------------------------------------------------
