...
def euler_to_quaternion(roll, pitch, yaw):
    ...
    Required because new TCP interface gives Euler, but script uses Quaternions.
    """
...
class Task5c(Node):
    def __init__(self):
       ...
       # --- FERTILIZER SERVO PARAMETERS ---
        ...
        self.dustbinOrientation = normalize_quaternion([-0.684, 0.726, 0.05, 0.008])
...
# --------------------------------------------------------------------------
    def tcp_pose_callback(self, msg):
        # CHANGED: Handle Float64MultiArray [x,y,z, r,p,y]
        if len(msg.data) >= 6:
            # 1. Position
            self.current_tcp_pos = np.array([msg.data[0], msg.data[1], msg.data[2]])
            # 2. Orientation (Convert Euler -> Quaternion)
            roll = msg.data[3]
            pitch = msg.data[4]
            yaw = msg.data[5]
...
# -------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if self.align_joint_to_pose(self.ferti_pose, 'shoulder', self.joint1, 0):
                ...
                self.phase = 'APPROACH_FERTILIZER'
# -------------------------------------------------------------------------------------------------------------------------------------
        elif self.phase == 'APPROACH_FERTILIZER':
            if self.current_tcp_pos is None or self.current_tcp_orient is None:
                self.get_logger().warn("Waiting for TCP pose/orientation...", throttle_duration_sec=1.0)
                return

            approach_position = self.ferti_pose + np.array([0.0, self.approachSideOffsetYmeters, 0.0])
            self.get_logger().info("Moving to fertilizer...", throttle_duration_sec=1.0)

            reached = self.servo_to_goal(
                self.current_tcp_pos, 
                self.current_tcp_orient,
                approach_position, 
                self.pickupOrientationQuaternion,
                self.fertilizerPositionTolerance, 
                self.fertilizerAngularToleranceRadians,
                self.fertilizerMaximumLinearVelocity, 
                self.fertilizerMaximumAngularVelocity
            )
            
            if reached:
                self.get_logger().info("Reached fertilizer approach point.")
                self.halt_motion()
                self.phase = 'MOVE_TO_FERTILIZER_CENTER'

# ---------------------------------------------------------------------------------------------------------------------------------------
        elif self.phase == 'MOVE_TO_FERTILIZER_CENTER':
            if self.current_tcp_pos is None or self.current_tcp_orient is None:
                self.get_logger().warn("Waiting for TCP pose/orientation...", throttle_duration_sec=1.0)
                return

            target_position = np.copy(self.ferti_pose)
            
            distance_to_center = float(np.linalg.norm(target_position - self.current_tcp_pos))
            is_close_enough = distance_to_center <= 0.015

            if is_close_enough:
                if self.settleStartTime is None:
                    self.get_logger().info(f"Centering on can... {distance_to_center:.3f} m")
                    self.settleStartTime = time.time()
                    self.halt_motion()
                    return

                if (time.time() - self.settleStartTime) > 0.3:
                    self.get_logger().info("Stable. Grabbing now.")
                    self.settleStartTime = None
                    self.phase = 'ATTACH_FERTI_PRE_WAIT'
                else:
                    self.get_logger().info(f"Holding... {distance_to_center:.3f} m", throttle_duration_sec=0.5)
                    self.halt_motion()
                return
            else:
                if self.settleStartTime is not None:
                    self.settleStartTime = None
                
                self.get_logger().info(f"Moving to center... {distance_to_center:.3f} m", throttle_duration_sec=1.0)
...
# --------------------------------------------------------------------------------------------------
            ...
            # Use updated force check logic
            if self.set_gripper_state('attach'):
            ...
            if reached:
                self.get_logger().info("Waiting reverse ...", throttle_duration_sec=1.0)
                if self.wait_for_timer(3):
                    self.get_logger().info("✓ Reversed fertilizer. Transitioning to PHASE_RETURN.")
                    self.phase = 'PHASE_RETURN_TO_INITIAL'


        elif self.phase == 'PHASE_RETURN_TO_INITIAL':
            # Check if we stored the pose in Phase 1
            if not hasattr(self, 'initial_cartesian_pos') or self.initial_cartesian_pos is None:
                self.get_logger().error("Initial Cartesian pose was not stored in Phase 1!")
                return

            self.get_logger().info("Servo-ing back to Initial Pose...", throttle_duration_sec=1.0)

            # Use servo_to_goal to move back to the start
            reached = self.servo_to_goal(
                self.current_tcp_pos,
                self.current_tcp_orient,
                self.initial_cartesian_pos,      # Target: Stored Initial Position
                self.initial_cartesian_orient,   # Target: Stored Initial Orientation
                0.05, # Position Tolerance (meters)
                self.fertilizerAngularToleranceRadians,  # Angular Tolerance (radians)
                self.fertilizerMaximumLinearVelocity,
                self.fertilizerMaximumAngularVelocity
            )
...
# =========================================================================
            ...
            self.wrist_orientation('down', 'MOVE_TO_EBOT')
# -----------------------------------------------------------------------
# =================================================================================================================
#             if you try this then remove the first hardcode pose of ebot bro   jab aman ready hoga 
# ===============================================================================================================
        elif self.phase == 'GEETTING_TF_EBOT': 
            ...
            if self.ebotWorldPosition is None:
                ...
                self.phase = 'MOVE_TO_EBOT'


# ----------------------------------------------------------------------------

        elif self.phase == 'MOVE_TO_EBOT':
            if self.ebotWorldPosition is None: return

            drop_position = self.ebotWorldPosition + np.array([self.dropOffsetXmeters, self.dropOffsetYmeters, self.dropHeightMetersAboveEbot])
            distance_to_drop = float(np.linalg.norm(drop_position - self.current_tcp_pos))

            is_at_drop = distance_to_drop <= self.dropPositionTolerance
            
            if is_at_drop:
                if self.settleStartTime is None:
                    self.get_logger().info(f"At eBot drop zone. Stabilizing... {distance_to_drop:.3f} m")
                    self.settleStartTime = time.time()
                    self.halt_motion()
                    return
                
                if (time.time() - self.settleStartTime) >= 0.3:
                    self.get_logger().info("Stable. Dropping can.")
                    self.settleStartTime = None
                    self.phase = 'PLACE_FERTILIZER'
                else:
                    self.get_logger().info(f"Holding... {distance_to_drop:.3f} m", throttle_duration_sec=0.5)
                    self.halt_motion()
                return
            else:
                if self.settleStartTime is not None:
                ...
                self.get_logger().info(f"Moving to eBot... {distance_to_drop:.3f} m", throttle_duration_sec=0.5)
                self.servo_to_goal(
                    self.current_tcp_pos, 
                    self.current_tcp_orient,
                    drop_position, 
                    self.horizontalCarryQuaternion,
                    self.dropPositionTolerance, 
                    self.fertilizerAngularToleranceRadians,
                    self.fertilizerMaximumLinearVelocity, 
                    self.fertilizerMaximumAngularVelocity
                )

        elif self.phase == 'PLACE_FERTILIZER':
            self.set_gripper_state('detach') # Removed object name
            self.get_logger().info("Can released over eBot. Waiting for fall...")
            self.phase = 'PLACE_FERTILIZER_WAIT'

        elif self.phase == 'PLACE_FERTILIZER_WAIT':
            if self.wait_for_timer(2):
                self.phase = 'RETRACT_FROM_EBOT'

        elif self.phase == 'RETRACT_FROM_EBOT':
            # Move straight up for safety
            if not hasattr(self, 'retractTargetPosition'):
                self.retractTargetPosition = self.current_tcp_pos + np.array([0.0, 0.0, 0.10])
            
            self.get_logger().info("Moving up, away from eBot.", throttle_duration_sec=1.0)

            reached = self.servo_to_goal(
                self.current_tcp_pos, 
                self.current_tcp_orient,
                self.retractTargetPosition, 
                self.horizontalCarryQuaternion,
                0.02, 
                self.fertilizerAngularToleranceRadians,
                self.fertilizerMaximumLinearVelocity, 
            ...
            if reached:
                self.get_logger().info("Clear of eBot. Transitioning to Fruits Tray.")
                self.halt_motion()
                if hasattr(self, 'retractTargetPosition'):
                    del self.retractTargetPosition
                
                # Transition to next phase (Fruits Tray)
                self.phase = 'MOVE_TO_FRUITS_TRAY_POS' 
                self.get_logger().info("DONE WITH FERTILIZER. Starting Fruit s.")
# ========================================================================
# ---------------------  Fruits Tray  ------------------------------
# ========================================================================
                
        elif self.phase == 'MOVE_TO_FRUITS_TRAY_POS':
            if self.current_tcp_pos is None: 
                self.get_logger().warn("Waiting for TCP pose go to for fruits...", throttle_duration_sec=1.0)
                return

            # 1. Move to the position only
            self.get_logger().info(f"Moving to fruit tray.", throttle_duration_sec=1.0)
            
            reached_pos = self.servo_to_goal(
                self.current_tcp_pos, self.current_tcp_orient,
                self.fruitHomePosition, self.fruitHomeOrientation,
                self.fruitTrayPositionTolerance, 
                self.fruitAngularToleranceRadians,
                self.fruitMaximumLinearVelocity,
                self.fruitMaximumAngularVelocity,
                checkOrientation=False # Ignore orientation for now
            )
            
            if reached_pos:
                self.get_logger().info("At tray. Fixing orientation.")
                self.halt_motion()
                self.phase = 'CORRECT_FRUIT_TRAY_ORIENTATION'
            return

        elif self.phase == 'CORRECT_FRUIT_TRAY_ORIENTATION':
            if self.current_tcp_pos is None: return

            # 2. At the correct position, now fix the orientation
            self.get_logger().info(f"Rotating wrist at tray...", throttle_duration_sec=1.0)

            reached_orientation = self.servo_to_goal(
                self.current_tcp_pos, self.current_tcp_orient,
                self.fruitHomePosition, self.fruitHomeOrientation,
                1.0, # Ignore position
                self.fruitAngularToleranceRadians,
                self.fruitMaximumLinearVelocity,
                self.fruitMaximumAngularVelocity,
                checkOrientation=True # Focus on orientation
            )

            if reached_orientation:
                self.get_logger().info("arm is steel on tray . Holding position.")
                self.halt_motion()
                self.phase = 'SETTLE_AT_FRUIT_TRAY'
            return
            
        elif self.phase == 'SETTLE_AT_FRUIT_TRAY':
            # 3. Hold the position for stability
            if self.settleStartTime is None:
                self.settleStartTime = time.time()
                self.halt_motion()
                self.get_logger().info(f"At fruit tray. Holding.")
                return

            self.halt_motion()
            if time.time() - self.settleStartTime >= 0.3:
                self.settleStartTime = None # Changed from del
                self.get_logger().info("Okay, starting fruit cleanup.")
                self.phase = 'PROCESS_BAD_FRUITS'
                self.current_fruit_index = 0 # Fixed variable name
            else:
                self.get_logger().info(f"Settling at tray...", throttle_duration_sec=0.5)
            return
        
# ---------------------------------------------------------------------------------------------------------------

        elif self.phase == 'PROCESS_BAD_FRUITS':
            if self.current_fruit_index >= len(self.badFruitTable):
                self.get_logger().info("All fruits done. Finishing up.")
                self.phase = 'DONE'
                return
            self.current_fruits_pose = self.badFruitTable[self.current_fruit_index]
            fruit_position = self.current_fruits_pose['pos']
            approach_position = fruit_position + np.array([0.0,0.0,self.fruitApproachOffsetZmeters])
            ...
            reached = self.servo_to_goal(
                ...
                self.fruitPositionTolerance,
                self.fruitAngularToleranceRadians,
                self.fruitMaximumLinearVelocity,
                self.fruitMaximumAngularVelocity
            )
            if reached:
                self.get_logger().info(f"Reached fruit {self.current_fruit_index + 1} approach point.")
                self.halt_motion()
                self.phase = 'LIFT_SAFE_FRUITS'



# ===============================================================================================================================  optinal if time ==========================================

        elif self.phase == 'MOVE_TO_FRUIT_CENTER':
            if self.current_tcp_pos is None or self.current_tcp_orient is None:
                self.get_logger().warn("Waiting for TCP pose/orientation...", throttle_duration_sec=1.0)
                return

            target_position = np.copy(self.current_fruits_pose['pos'])+np.array([0.0,0.0,0.065])
...
# ==========================================================================================================
        ...
        elif self.phase == 'LIFT_SAFE_FRUITS':
            fruit_record = self.badFruitTable[self.current_fruit_index]
            fruit_position = fruit_record['pos']
            approach_position = fruit_position + np.array([0.0,0.0,0.1])


            # ##################################        for collision to dustbin if needed 
            # approach_position = self.fruitHomePosition
            # ################################
            reached = self.servo_to_goal(
                self.current_tcp_pos,
                self.current_tcp_orient,
            ...
            if reached:
                self.get_logger().info(f"Lifted fruit {self.current_fruit_index + 1} safely.")
                self.halt_motion()
                self.current_fruit_index += 1
                self.phase = 'MOVE_TO_DUSTBIN'
       # 
        elif self.phase == 'MOVE_TO_DUSTBIN':
            if self.current_tcp_pos is None: return

            self.get_logger().info("Moving to Dustbin...", throttle_duration_sec=1.0)
            approach_position = self.dustbinPosition+np.array([0.0,0.1,0.0])

            # Move to the Dustbin Position and Orientation
            reached = self.servo_to_goal(
                self.current_tcp_pos, 
                self.current_tcp_orient,
                approach_position, 
            ...
            if reached:
                ...
                self.halt_motion()
                self.phase = 'DONE' # Or whatever final phase you want
            return
            
        elif self.phase == 'DONE':
            self.get_logger().info("all task done bro look at the tcp pose", throttle_duration_sec=2.0)
            pass
