...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------
                    ...
                    # Confirmed fruit-pickup orientation from real arm logs:
                    # gripper pointing down, [qx, qy, qz, qw]
                    FRUIT_PICKUP_QUAT = np.array([0.70182645, -0.71100347, -0.01433901, 0.04132911])

                    reached = self.move_to_tcp_pose_v3(
                        target_pos  = self.hover_target,
                        target_quat = FRUIT_PICKUP_QUAT,
                        tol_xy      = 0.015,
                        tol_z       = 0.020,
                        tol_rot     = 0.05,
                        slow        = False
                    ...
                    if reached:
                        self.get_logger().info(
                            f"✓ Fruit hover reached with correct orientation. "
                            f"orient={self.current_tcp_orient} pos={self.current_tcp_pos} "
                            f"fruit={self.current_fruits_pose}")
                        self.phase_initialized = False
...
# --------------------------------------------------------------PHASE_ALIGN_TO_FERTI------------------------------------------------------
            ...
            reached = self.move_to_tcp_target(self.lift_target,tol=0.03,slow=True)
