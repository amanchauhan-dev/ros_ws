...
# =============================main loop======================================================================
    def main_loop(self):
        if self.phase == 'START':
            ...
            self.get_logger().info(f"intial {self.current_tcp_orient}  euler {self.current_euler_angel}, pose current {self.current_tcp_pos} ,  \n jointState {self.joint_pos}")
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if self.align_joint_to_pose(self.ferti_unload_pose, 'shoulder', self.joint_names_list[0], 0):
                self.get_logger().info(f" {self.current_tcp_orient}  euler {self.current_euler_angel}, pose current {self.current_tcp_pos} ,  \n jointState {self.joint_pos}")
...
# ------------------------------------------------------------------------------------------------------------------------------------
            ...
            if self.wait_for_timer(0.5):
                ...
                self.get_logger().info(" magnet start check")
...
# ------------------------------------------------------------------------------------------------------
            ...
            if reached :
                self.stop_all()
                self.get_logger().info(f"  ferti unload  final {self.current_tcp_orient} ,euler {self.current_euler_angel}, pose current {self.current_tcp_pos} ,   jointState {self.joint_pos}")
                self.get_logger().info("Lift Complete.")
                self.phase_initialized = False
                self.phase = 'ATTACH_TARGET_UNLOAD_WAIT'
...
# ---------------------------------------------------------------------------------------------------------------------------------------
            ...
            self.phase = 'LIFT_TARGET_UNLOAD'
             ...
             if not self.phase_initialized:
                ...
                self.lift_unload_target[2] += 0.05
                # self.lift_unload_target[0] -= 0.05
...
# --------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if self.move_to_tcp_target_v2(self.fruitHomePosition, tol=0.02, slow=False):
...
# ----------------------------------------------------------------------------------------------------------
            ...
            if not self.phase_initialized :
                ...
                self.target_dustbin[2] += 0.08
                self.target_dustbin[0] -= 0.05
