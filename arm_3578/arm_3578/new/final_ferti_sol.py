...
# =====================================extra function ====================================
    def compute_wrist_for_orientation(self, target_quat):
        """
        Computes wrist_1 and wrist_2 delta commands to align wrist_3_link
        to a desired quaternion orientation — purely orientation-driven,
        NOT based on joint angle formulas.

        Strategy:
            1. Lookup wrist_3_link TF to get current orientation
            2. Compute quaternion error: q_err = q_target * inv(q_current)
            3. Decompose error into pitch (wrist_1) and roll (wrist_2) axes
            4. Return joint deltas proportional to error

        Args:
            target_quat : np.array [qx, qy, qz, qw]
        Returns:
            dict  {'wrist_1_joint': float, 'wrist_2_joint': float}
            None  if TF not available yet
        """
        # ── 1. Get current wrist_3 orientation from TF ──────────────────────
        try:
            ...
            self.get_logger().warn(f"[WRIST_TF] Cannot get wrist_3 TF: {e}")
            return None

        o = tf.transform.rotation
        q_current = normalize_quaternion([o.x, o.y, o.z, o.w])
        q_target  = normalize_quaternion(target_quat)

        # ── 2. Quaternion error ──────────────────────────────────────────────
        # q_err = q_target * inv(q_current)
        q_inv = conjugate_quaternion(q_current)
        q_err = multiply_quaternion(q_target, q_inv)
        ...
        # Error axis-angle
        xyz_err   = q_err[:3]                                        # rotation axis * sin(half_angle)
        error_mag = np.linalg.norm(xyz_err)
        deg_err   = np.degrees(2.0 * np.arcsin(np.clip(error_mag, 0, 1)))

        self.get_logger().info(
            f"[WRIST_TF] q_current={np.round(q_current,3)} "
            f"q_target={np.round(q_target,3)} "
            f"q_err={np.round(q_err,3)} "
            f"total_err={deg_err:.1f}deg",
            throttle_duration_sec=0.5
        )

        if error_mag < 0.03:   # ~3.4 deg — aligned
            self.get_logger().info(f"[WRIST_TF] ✓ Orientation aligned! err={deg_err:.1f}deg")
            return {'wrist_1_joint': self.joint_pos.get('wrist_1_joint', 0.0),
                    'wrist_2_joint': self.joint_pos.get('wrist_2_joint', 0.0)}

        # ── 3. Project error onto wrist axes ────────────────────────────────
        # In base_link frame:
        #   wrist_1 controls PITCH  → Y axis  → q_err[1]  (qy component)
        #   wrist_2 controls ROLL   → X axis  → q_err[0]  (qx component)
        # These are approximate projections — works well for small errors
        pitch_err = q_err[1]   # wrist_1 axis
        roll_err  = q_err[0]   # wrist_2 axis

        # ── 4. P-controller: current_angle + correction ──────────────────────
        KP_W1 = 2.5
        KP_W2 = 2.5
        MAX_DELTA = 0.4   # rad — max single-step correction

        w1_correction = float(np.clip( KP_W1 * pitch_err, -MAX_DELTA, MAX_DELTA))
        w2_correction = float(np.clip( KP_W2 * roll_err,  -MAX_DELTA, MAX_DELTA))

        w1_target = self.joint_pos.get('wrist_1_joint', 0.0) + w1_correction
        w2_target = self.joint_pos.get('wrist_2_joint', 0.0) + w2_correction

        # Safety clamp — keep within UR5 joint limits
        w1_target = float(np.clip(w1_target, -2.0, 2.0))
        w2_target = float(np.clip(w2_target,  0.5, 2.5))

        self.get_logger().info(
            f"[WRIST_TF] pitch_err={np.degrees(pitch_err*2):.1f}deg "
            f"roll_err={np.degrees(roll_err*2):.1f}deg | "
            f"w1_target={w1_target:.4f} w2_target={w2_target:.4f}",
            throttle_duration_sec=0.4
        )

        return {
            'wrist_1_joint': w1_target,
            'wrist_2_joint': w2_target,
        }
# ------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------------------------
            ...
            result = self.compute_wrist_for_orientation(
                self.pickupFertiOrientationQuaternion   # [0.707, 0.028, 0.034, 0.707]
            )

            if result is None:
                return  # TF not ready yet

            speed_scale = {'wrist_1_joint': 0.5, 'wrist_2_joint': 0.5}

            # Check if already aligned (result == current joints means no correction needed)
            w1_curr = self.joint_pos.get('wrist_1_joint', 999)
            w2_curr = self.joint_pos.get('wrist_2_joint', 999)
            already_aligned = (
                abs(result['wrist_1_joint'] - w1_curr) < 0.03 and
                abs(result['wrist_2_joint'] - w2_curr) < 0.03
            )

            if already_aligned:
                self.get_logger().info("[ORIENT] ✓ Wrist aligned to target quaternion.")
                self.phase = 'PHASE_FINAL_APPROACH_WAIT'
                return

            reached = self.move_joint_group(result, speed_scale)
