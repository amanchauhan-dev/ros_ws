...
# =====================================extra function ====================================
    def compute_wrist_for_orientation(self, target_quat):
        """
        Returns angular velocity twist commands (NOT joint targets).
        Called every 20ms timer tick — arm continuously corrects until aligned.
        Returns: (aligned: bool, deg_err: float)
        """
        try:
            ...
            self.get_logger().warn(f"[WRIST_TF] {e}", throttle_duration_sec=1.0)
            return False, 999.0
        ...
        deg_err   = float(np.degrees(2.0 * np.arcsin(np.clip(error_mag, 0, 1))))

        self.get_logger().info(
            f"[ORIENT] err={deg_err:.1f}deg | "
            f"q_curr={np.round(q_current,3)} | "
            f"q_err={np.round(q_err,3)}",
            throttle_duration_sec=0.3
        )

        # Convergence — tight tolerance
        if error_mag < 0.05:   # ~5.7 deg
            self.stop_all()
            self.get_logger().info(f"[ORIENT] ✓ Aligned! err={deg_err:.1f}deg")
            return True, deg_err

        # Angular velocity P-controller — publish twist directly
        # No joint targets, no move_joint_group — just pure rotation
        if deg_err > 30.0:
            KP = 2.0
        elif deg_err > 10.0:
            KP = 1.5
        ...
        # xyz_err is the rotation axis * sin(half_angle) in base frame
        ang_vel = np.clip(KP * xyz_err, -MAX_ANG, MAX_ANG)

        self.get_logger().info(
            f"[ORIENT] ang_vel=[{ang_vel[0]:.3f},{ang_vel[1]:.3f},{ang_vel[2]:.3f}]",
            throttle_duration_sec=0.3
        )

        # Pure rotation, no translation
        self.publish_twist([0.0, 0.0, 0.0], ang_vel)
        return False, deg_err
...
# ------------------------------------------------------------------------------------------------------------------------
            ...
            aligned, deg_err = self.compute_wrist_for_orientation(
                self.pickupFertiOrientationQuaternion
            )
            if aligned:
                self.get_logger().info(
                    f"[ORIENT] ✓ Wrist aligned. Moving to final approach."
                )
                self.phase = 'PHASE_FINAL_APPROACH_WAIT'
            # else: loop continues every 20ms, arm keeps rotating
# --------------------------------------------------------------------------------------------------------------------------------------------------------------
