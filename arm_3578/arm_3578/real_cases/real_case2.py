...
class Task6(Node):
    def __init__(self):
        ...
        self.base_kp = 1.0
        self.base_max_speed = 1.0        # Linear Max
        ...
        # -----------------------------------------------------------------------------------
        self.dustbinPosition = np.array([-0.75, 0.210, 0.316])

        # -----------------------------------------------------------------------------------
        self.phase_initialized = False
        self.current_fruits_pose = None
        self.wait_start_time = None
        self.wrist1_delta_down = 1.47  # radinal in the degree  near 90
...
# ---------------------------------------------------------------------------------
    def orient_to_target(self, target_roll=None, target_pitch=None, target_yaw=None, tol=0.05):
        """
        Rotates TCP to target Euler angles. Pass None to ignore that axis.
        Uses adaptive P-controller with 3 speed zones — tapers naturally to zero.
        No hard minimum speed floor → clean deceleration, no oscillation.

        Examples:
            orient_to_target(target_pitch=np.pi/2)
            orient_to_target(target_yaw=np.pi/2, tol=0.03)
            orient_to_target(target_pitch=0.0, target_yaw=np.pi/2)
        """
        if self.current_euler_angel is None:
            self.get_logger().warn("orient_to_target: no TCP euler data yet", throttle_duration_sec=1.0)
            return False

        current_roll  = self.current_euler_angel[0]
        current_pitch = self.current_euler_angel[1]
        current_yaw   = self.current_euler_angel[2]

        # ── Errors (only for active axes) ─────────────────────────────────
        err_roll  = self.norm(target_roll  - current_roll)  if target_roll  is not None else 0.0
        err_pitch = self.norm(target_pitch - current_pitch) if target_pitch is not None else 0.0
        err_yaw   = self.norm(target_yaw   - current_yaw)   if target_yaw   is not None else 0.0

        roll_ok  = (target_roll  is None) or (abs(err_roll)  < tol)
        pitch_ok = (target_pitch is None) or (abs(err_pitch) < tol)
        yaw_ok   = (target_yaw   is None) or (abs(err_yaw)   < tol)

        # ── Adaptive gain — based on largest active error ──────────────────
        # Only consider errors from axes we are actually controlling
        active_errors = []
        if target_roll  is not None: active_errors.append(abs(err_roll))
        if target_pitch is not None: active_errors.append(abs(err_pitch))
        if target_yaw   is not None: active_errors.append(abs(err_yaw))

        error_mag = max(active_errors) if active_errors else 0.0

        if error_mag > 0.3:        # far   → fast approach
            kp_rot, max_rot = 3.0, 0.80
        elif error_mag > 0.1:      # mid   → controlled
            kp_rot, max_rot = 2.5, 0.40
        else:                      # close → natural taper, NO floor
            kp_rot, max_rot = 2.0, 0.15

        # ── Debug log (active axes only) ──────────────────────────────────
        log_parts = [f"zone={'FAR' if error_mag>0.3 else 'MID' if error_mag>0.1 else 'NEAR'} "
                    f"kp={kp_rot} max={max_rot} mag={np.degrees(error_mag):.1f}°"]
        if target_roll  is not None: log_parts.append(f"roll={np.degrees(current_roll):+6.1f}° err={np.degrees(err_roll):+5.1f}°({'✓' if roll_ok  else '✗'})")
        if target_pitch is not None: log_parts.append(f"pitch={np.degrees(current_pitch):+6.1f}° err={np.degrees(err_pitch):+5.1f}°({'✓' if pitch_ok else '✗'})")
        if target_yaw   is not None: log_parts.append(f"yaw={np.degrees(current_yaw):+6.1f}° err={np.degrees(err_yaw):+5.1f}°({'✓' if yaw_ok   else '✗'})")
        self.get_logger().info(f"[ORIENT] {' | '.join(log_parts)}", throttle_duration_sec=0.4)
        ...
        if roll_ok and pitch_ok and yaw_ok:
            ...
            self.get_logger().info(
                f"✓ ORIENT REACHED | "
                f"roll={np.degrees(current_roll):.1f}° "
                f"pitch={np.degrees(current_pitch):.1f}° "
                f"yaw={np.degrees(current_yaw):.1f}°"
            )
            return True
                # ── P-controller per axis (signed, clipped, no floor) ─────────────
        def p_ctrl(err):
            cmd = kp_rot * err
            return max(min(cmd, max_rot), -max_rot)

        wx = p_ctrl(err_roll)  if target_roll  is not None else 0.0
        wy = p_ctrl(err_pitch) if target_pitch is not None else 0.0
        wz = p_ctrl(err_yaw)   if target_yaw   is not None else 0.0
...
# =============================main loop======================================================================
    def main_loop(self):
        ...
        # ✅ FIXED VERSION
        elif self.phase == 'ORENTATION_WALA_PICKUP':
            reached = self.orient_to_target(
                # target_roll  = 0.3,   # bug 1 fixed — radians
                target_pitch = -0.49,              # already radians, fine
                # target_yaw   =  5.02,             # already radians, fine
                tol          = 0.05
            )
            if reached:                            # bug 2 fixed — wait until done
                self.phase = 'PHASE_FINAL_APPROAtttCH_WAIT'   # bug 3 fixed — spelling
...
# ------------------------------------------------------------------------------------------------------------------------
            ...
            if self.move_joint_group(targets, speed_scale):
                self.get_logger().info(f" orentaion wala hai  {self.current_euler_angel}  quatration {self.current_tcp_orient} , pose current {self.current_tcp_pos} , frti {self.ferti_pose}")
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            #     self.final_ferti_target, tol_xy=0.008, tol_z=0.010, slow=True
            # )
            # ==========================================================================================================================
            if reached:
                self.stop_all()
...
# -----------------------------------------------------------------------------------------------------------------------
            ...
            if self.move_to_tcp_target(self.reverse_target, tol=0.05, slow=True):
...
# -------------------------------------------------------------------------------------------------------------------------------
            ...
            if self.move_joint_to_angle(self.target_wrist_val, joint_name, joint_idx):
                ...
                self.phase_initialized = False
                self.phase = 'GETTING_TF_EBOT_WAIT'
# --------------------------------------------------------------------------------------------------------------------

# -------------------------------------------------------------------------------------------------------------------------
        elif self.phase == 'GETTING_TF_EBOT_WAIT':
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if self.move_to_tcp_target(self.Retract_target, tol=0.005):
                self.get_logger().info("Lift Complete.")
                self.phase_initialized = False
...
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            if self.move_joint_to_angle(self.target_wrist_val, joint_name, joint_idx):
                ...
                self.phase_initialized = False
                self.phase = 'FERTI_UNLOAD_ALING_WAIT'   #  < ========  current not move for the certain angel  just aling and moved



# --------------------------------------------------------------------------------------------------------------------------------------------------------------
# ==========================================================================================================================================================================================================
#  this used when the arm is not operate properly  the i move the angel  of joint for the perfect orentation

        elif self.phase == 'MOVED_TO_CERTAIN_ANGEl':
            if not self.phase_initialized :
                self.current_joint_pose = self.joint_pos.copy()
                self.targets = {

                    'shoulder_lift_joint': self.current_joint_pose['shoulder_lift_joint'],    # here need to change add or subtract need if used
                    'elbow_joint': self.current_joint_pose['elbow_joint'],
            ...
            if self.move_joint_group(self.targets, self.speed_scale):
                self.phase_initialized  = False
                self.get_logger().info(f"gripper down phase {self.current_tcp_orient}  euler {self.current_euler_angel}, pose current {self.current_tcp_pos} ,  \n jointState {self.joint_pos}")
                self.get_logger().info("move upto certain for the ebot which suit the orenattion for the target")
...
# ----------------------------------------------------------------------------------------------------------------------------------------
            ...
            if not self.phase_initialized:
                 ...
                 self.ferti_unload_target[2] += 0.10
                 self.phase_initialized = True
...
# ------------------------------------------------------------------------------------------------------
            ...
            if not self.phase_initialized:
                 ...
                 self.ferti_unload_target_final[2] -= 0.05
