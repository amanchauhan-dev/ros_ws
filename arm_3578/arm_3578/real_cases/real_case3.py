...
# -----------------------------------------------------------------------------------------------------------------------
            ...
            targets = {
                ...
                'elbow_joint': self.temp['elbow_joint'] + 0.077,
                'wrist_1_joint': self.temp['wrist_1_joint'] - 0.0683,
            }
...
# -----------------------------------------------------------------------------------------------------------------------
            ...
            targets = {
                ...
                'shoulder_lift_joint': self.temp['shoulder_lift_joint'] + 0.2737,
                'elbow_joint': self.temp['elbow_joint'] - 0.30172,
                'wrist_1_joint': self.temp['wrist_1_joint'] + 0.02667,
            }
...
# ------------------------------------------------------------------------------------------------------------------------------------------
            ...
            self.phase = 'REVRSE_TO_INIT_FRUITS'

# -----------------------------------------------------------------------------------------------------------------------------------------------------------------
        elif self.phase == 'REVRSE_TO_INIT_FRUITS':
            ...
            if self.move_joint_group(targets, speed_scale):
                self.get_logger().info("Fertilizer alignment restored (FAST).")
                self.phase = 'PHASE_SAFE_LIFT_SHOULDER_4_RACK_WAIT'

# --------------------------------------------------------------------------------------------------------------------------
        elif self.phase == 'PHASE_SAFE_LIFT_SHOULDER_4_RACK_WAIT':
            if self.wait_for_timer(2.0):
                self.phase ='PHASE_SAFE_LIFT_SHOULDER_4_RACK'

# ----------------------------------------------------------------------------------------------------------------------------------------
        elif self.phase == 'PHASE_SAFE_LIFT_SHOULDER_4_RACK':
            if self.safe_lift_angle is None:
                current_val = self.joint_pos[self.joint_names_list[1]]
                self.safe_lift_angle = current_val - 0.2 
                self.get_logger().info(f"Lifting Shoulder...  Note only for the gazebo because the elbow get colied with the fruits tray   Target: {self.safe_lift_angle:.2f}")
...
# ----------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            reached = self.move_to_tcp_target(final_target,tol=0.01,slow=True)
...
# -----------------------------------------------------------------------------------------------------------------------
             ...
             targets = {
                ...
                'elbow_joint': self.temp['elbow_joint'] + 0.32172,
                'wrist_1_joint': self.temp['wrist_1_joint'] - 0.3208,
            }
